...
export default async function LabsPage() {
  ...
  // Get file/url/quiz/assignment lessons — join via enrollments (no array param needed)
  ...
  // Resources attached to lessons — try/catch in case table doesn't exist yet
  let resources: any[] = []
  try {
    resources = await sql`
    ...
    <div className="space-y-6 max-w-4xl mx-auto">
                ...
                <div key={item.lesson_id} className="flex items-center gap-3.5 px-4 py-3.5">
                  ...
                  {url && (
                    <a href={url} target="_blank" rel="noopener noreferrer"
                      className="flex items-center gap-1.5 text-xs font-medium text-bloomy-600 bg-bloomy-50 hover:bg-bloomy-100 px-3 py-1.5 rounded-lg flex-shrink-0">
                ...
                <div key={String(courseId)} className="bg-white rounded-2xl border border-gray-100 overflow-hidden">
                  ...
                  {items.map((lab: any) => (
                    <Link key={lab.lesson_id} href={`/learn/${lab.course_slug}?lesson=${lab.lesson_id}`}
                      className="flex items-center gap-3.5 px-4 py-3.5 hover:bg-gray-50 border-b border-gray-50 last:border-0 group">
                      <div className={`w-9 h-9 rounded-xl flex items-center justify-center flex-shrink-0 ${
                        lab.completed ? 'bg-green-100' : lab.type === 'quiz' ? 'bg-purple-50' : 'bg-orange-50'
                      }`}>
                        {lab.completed ? <span className="text-base">✅</span>
                          : lab.type === 'quiz' ? <HelpCircle className="w-4 h-4 text-purple-600" />
                      ...
                      <span className={`text-xs font-medium px-2.5 py-1 rounded-full flex-shrink-0 ${
                        lab.completed ? 'bg-green-50 text-green-700'
                          : lab.type === 'quiz' ? 'bg-purple-50 text-purple-700'
      ...
      {fileItems.length === 0 && resources.length === 0 && labItems.length === 0 && (
        <div className="bg-white rounded-2xl border border-gray-100 p-12 text-center">
          ...
          <p className="font-medium text-gray-600 mb-1">No materials yet</p>
          <p className="text-sm text-gray-400">Your instructor will upload course materials and labs here.</p>
        </div>
      )}
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
          {courses.map((c: any) => (
            <Link key={c.course_id} href={`/learn/${c.slug}`}
              className="flex items-center gap-3 bg-white rounded-xl border border-gray-100 p-3.5 hover:shadow-sm hover:border-bloomy-100 group">
