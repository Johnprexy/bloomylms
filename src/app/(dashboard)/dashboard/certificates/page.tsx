...
export default async function CertificatesPage() {
  ...
  const certs = await sql`
    SELECT
      ce.id, ce.certificate_number, ce.issued_at, ce.pdf_url, ce.status,
  ...
  // Completed courses that don't have a cert yet (can generate)
  const completedNoCert = await sql`
    SELECT
      e.id as enrollment_id,
      c.id as course_id, c.title, c.slug, c.certificate_enabled,
      cat.icon as category_icon
    FROM enrollments e
    ...
    <div className="max-w-4xl mx-auto space-y-7">
      <div className="flex items-center gap-3">
        ...
        <div>
          ...
          <p className="text-gray-500 text-sm mt-0.5">{certs.length} certificate{certs.length !== 1 ? 's' : ''} earned</p>
        </div>
      </div>
      ...
      {completedNoCert.length > 0 && (
        <div>
          <h2 className="text-sm font-bold text-gray-700 mb-3 flex items-center gap-2">
            <Award className="w-4 h-4 text-yellow-500" /> Ready to Generate
          </h2>
            ...
            {completedNoCert.map((c: any) => (
              <div key={c.enrollment_id} className="bg-yellow-50 border border-yellow-200 rounded-2xl p-4 flex items-center gap-3">
                ...
                <div className="flex-1 min-w-0">
                  ...
                  <p className="text-xs text-gray-500">Course completed — certificate ready</p>
                </div>
                <form action="/api/certificates/generate" method="POST">
                  <input type="hidden" name="enrollment_id" value={c.enrollment_id} />
                  <button type="submit"
                    className="text-xs font-semibold bg-yellow-400 hover:bg-yellow-500 text-yellow-900 px-3 py-1.5 rounded-lg transition-colors flex-shrink-0">
                    Generate
                  </button>
                </form>
              </div>
      ...
      {certs.length > 0 ? (
        <div>
          <h2 className="text-sm font-bold text-gray-700 mb-3 flex items-center gap-2">
            <CheckCircle className="w-4 h-4 text-green-500" /> Earned Certificates
          </h2>
          <div className="grid sm:grid-cols-2 gap-4">
            {certs.map((cert: any) => (
              <div key={cert.id} className="bg-white rounded-2xl border border-gray-100 overflow-hidden shadow-sm hover:shadow-md transition-shadow">
                ...
                <div className="h-32 bg-gradient-to-br from-bloomy-600 via-blue-600 to-purple-700 relative p-5 flex flex-col justify-between">
                  <div className="flex items-start justify-between">
                    <div className="w-10 h-10 bg-white/20 rounded-xl flex items-center justify-center">
                      <span className="text-2xl">{cert.category_icon || '🏆'}</span>
                    </div>
                    <Award className="w-7 h-7 text-yellow-300 opacity-90" />
                  </div>
                  <div>
                ...
                <div className="p-4">
                  ...
                  <div className="flex items-center justify-between">
                    <p className="text-xs font-mono text-gray-400 truncate">{cert.certificate_number}</p>
