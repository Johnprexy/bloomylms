...
export default function CoursePlayer({ course, modules, enrollment, lessonProgress, userId, userName = '' }: Props) {
      ...
      <div className="flex-1 flex flex-col overflow-hidden">
        ...
        <div className="bg-gray-800 border-b border-gray-700 flex items-center px-4 gap-1">
          {[
            ...
            ...(resourceCount > 0 ? [{ id: 'resources', label: `Resources (${resourceCount})` }] : []),
          ].map(tab => (
            <button key={tab.id} onClick={() => setActiveTab(tab.id as any)}
              className={cn('text-xs font-medium px-3 py-3 border-b-2 transition-colors',
            ...
            <div className="max-w-4xl mx-auto p-4 lg:p-8">
                ...
                <>
                  {currentLesson.type === 'video' && currentLesson.video_url && (
                    <div className="rounded-xl overflow-hidden mb-6 bg-black aspect-video">
                      <iframe src={currentLesson.video_url} allow="autoplay; fullscreen; picture-in-picture" className="w-full h-full" title={currentLesson.title} />
                    </div>
                  )}
              ...
              {activeTab === 'discussions' && (
                <div className="bg-gray-800 rounded-xl p-6 text-gray-300 text-sm">
                  <p>Discussions for this lesson will appear here.</p>
                </div>
              )}
                ...
                <div className="space-y-3">
                  {currentLesson.resources.map((r: any) => (
                    <a key={r.id} href={r.url} target="_blank" rel="noopener noreferrer"
                      ...
                      <div className="w-10 h-10 bg-bloomy-600/20 rounded-lg flex items-center justify-center text-bloomy-400">
              ...
              <div className="flex items-center justify-between gap-4 pt-6 mt-6 border-t border-gray-700">
                <button onClick={() => prevLesson && setCurrentLesson(prevLesson)} disabled={!prevLesson}
                  className="flex items-center gap-2 text-sm text-gray-400 hover:text-white disabled:opacity-40 disabled:cursor-not-allowed">
          ...
          <div className="flex-1 overflow-y-auto">
            {modules.map((module: any) => (
              <details key={module.id} className="group" open>
                <summary className="flex items-center justify-between px-4 py-3 cursor-pointer bg-gray-800 sticky top-0 border-b border-gray-700 select-none">
                  <div>
                    <p className="text-xs font-semibold text-gray-300">{module.title}</p>
                    <p className="text-xs text-gray-500 mt-0.5">
                      {module.lessons?.filter((l: any) => progress[l.id]).length || 0}/{module.lessons?.length || 0} done
                    </p>
                  </div>
                ...
                {module.lessons?.map((lesson: any) => {
                  ...
                  return (
                    <button key={lesson.id} onClick={() => { setCurrentLesson(lesson); setActiveTab('content') }}
